...
const { data: targetProfile } = useReadProfile(profileId, {
...
<template>
    <LayoutSidebar title="Beheer" show-back-button @click:back="goBack">
        <div class="sidebar-nav-group">
            <LayoutSidebarItem
                :loading="isLoading"
                :to="{ name: 'manage.dashboard', params: { profileId: props.profileId } }"
                :active="route.name === 'manage.dashboard'">
                <template #img>
                    <EntityAvatar class="h-full w-full" :image="profile.avatar?.url" :icon="faHome">
                    </EntityAvatar>
                </template>
            ...
            <LayoutSidebarItem
                :loading="isLoading"
                :to="{ name: 'manage.locations', params: { profileId: props.profileId } }"
        ...
        <template v-if="isLoading || institutionMemberships?.length">
            <h3 class="sidebar-section-title">Organisaties</h3>
        ...
        <template v-if="isLoading || authorityMemberships?.length">
            <h3 class="sidebar-section-title">Groepen</h3>
            ...
            <div class="sidebar-nav-group">
                ...
                <LayoutSidebarItem
                    ...
                    :key="m.authority.id"
                    :to="{ name: 'manage.authority', params: { authorityId: m.authority.id } }">
                    <template #img>
                        <EntityAvatar
                            class="h-full w-full"
