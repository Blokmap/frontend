 ...
 */
export function isOpeningAtFullOccupancy(opening: OpeningTime): boolean {
    if (!opening.stats) return false;
    return opening.stats.reservationCount >= opening.seatCount;
}
 ...
 */
export function canInteractWithOpening(opening: OpeningTime): boolean {
    if (isDateInPast(opening.day)) return false;
    if (isOpeningAtFullOccupancy(opening)) return false;
